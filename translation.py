...
def main(args):
    ...
    #model = UNet2DModel.from_config(os.path.join(model_dir,"config.json"))
    ...
    #loaded_state_dict = load_file("/checkout/model_epoch_{args.selected_epoch}.safetensors")
    #model.load_state_dict(loaded_state_dict).to(args.device)
    if args.input_domain=="CT" and args.output_domain=="MRI":
        model_dir=f"/workspace/checkout/model_epoch_{args.selected_epoch}/unet/"
    elif args.input_domain=="MRI" and args.output_domain=="CT":
        model_dir=f"/workspace/checkout/mr_2_ct/model_epoch_{args.selected_epoch}/unet/"
    ...
    with torch.no_grad():
        if args.by_volume:
            ...
            for n, volume in enumerate(unique_volume_partition):
                ...
                for i, row in df_by_volume.iterrows():
                    ...
                    while not_pass_flag:
                        ...
                        image = randn_tensor(image_shape, generator=generator, device=config.device)
            ...
            slice_list = sorted(df_val_meta["contour_name"].to_list())
            slice_split = np.array_split(slice_list, args.num_partition)
            slice_partition = slice_split[args.partition]

            df_by_slice = sorted(df_val_meta[df_val_meta["contour_name"].isin(slice_partition)])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--seed', type=int, default=112, help="seeds for random noise generator")
    ...
    parser.add_argument('--selected_epoch', type=int, default=10, help="specifiy the epoch to load the checkpoints")
    ...
    parser.add_argument('--by_volume', action='store_false',default=False, help="specify if the translation is performed volume by volume")
