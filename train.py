...
def save_model(model,path:str=None):
    save_file(model.state_dict(),path+"model_weights.safetensors")
...
def main(args):
    ...
    train_dataset = ContourDiffDataset(
        df_train_meta.sample(n=2500,random_state=42), 
    ...
    for epoch in range(config.num_epochs):
        ...
        epoch=epoch+10 #for continue traning naa
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            noise_pred = model(sample=noisy_images,timestep=timesteps, return_dict=False)
        ...
        if (epoch == 0) or (epoch + 1) % config.save_model_epochs == 0 or epoch == config.num_epochs - 1:
            #save_model(model=model,path=args.output_dir+f"/model_epoch_{epoch}.safetensors")
            pipeline.save_pretrained(os.path.join(args.output_dir, f"model_epoch_{epoch + 1}"))
if __name__ == "__main__":
    ...
    parser.add_argument('--check_point', type=str, default=None, help="directory to load checkpoint")
